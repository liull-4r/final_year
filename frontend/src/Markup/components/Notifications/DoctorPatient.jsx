...
const DoctorPatient = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:9000/detection/doctorpatientmessagenotification/?patient_id=${user?.user_id}`
        );
  ...
  const markAsRead = async (notificationId) => {
    try {
      await axios.patch(
        `http://localhost:9000/detection/doctorpatientmessagenotification/${notificationId}/`,
        {
          read: true,
        }
      );
